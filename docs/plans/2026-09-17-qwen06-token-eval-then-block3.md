...
1. [ ] 新增独立控制器和失败测试，沿用现有评测/训练/恢复检查组件。
2. [ ] 校验旧runtime、模型/数据/grader与Token checkpoint；所有新产物写新目录。
3. [ ] 完成正式Block3配置生成和配对差异审计，尚不使用GPU训练。
4. [ ] nohup启动顺序队列，确认Token Step200完整评测实际进入GPU生成。
