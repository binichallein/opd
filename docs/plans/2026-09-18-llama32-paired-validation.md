...
1. [x] 测试先行，新增 Llama 模板/停止符适配；Qwen 回归测试不变。
2. [x] ModelScope 固定 revision 下载及26文件/词表核验，不占 GPU。
3. [x] 已实现等待门禁：Sep17v2 queue_state=complete、所有三步完整评测验收及旧输入保护验证；失败/中断则停止，不抢卡、不自动重试。前驱尚未完成。
...
7. [x] 测试、复查、GitHub 同步、不可变部署、启动等待队列并核对 PID/日志/无 Llama GPU 占用。PID297837，runtime f2d148e，状态waiting_predecessor（2026-09-18 02:32北京时间）。
