...
- [x] 新提示协议及测试。
- [x] 自动训练/评测控制器及测试。
- [x] 独立复核、实际环境回归和不可变部署：be736b5，ml2上438项通过，Git历史对照单测本地通过。
- [x] 服务端队列启动、Block3恢复门禁和正式训练启动确认：2026-09-22 00:07:30启动正式Block3。
- [x] Block3完整评测、原始学生评测、Token训练和完整评测：2026-09-22 20:29北京时间全部完成。
  Token原始启动失败后使用独立恢复控制器，冻结训练代码不变；
  [最终分项结果与完整归档](../results/2026-09-22-qwen17-instruct-final.md)。

2026-09-21 23:38北京时间服务端队列已启动，控制器PID1896918、PPID1、SID1896918。
此时处于启动预检，不代表GPU更新/恢复门禁或正式训练已经通过。
部署固定为 `be736b5fac4f26ff59f4e2c21abafc456c2503f6`；后续文档提交不替换运行代码。
