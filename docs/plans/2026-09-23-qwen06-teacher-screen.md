...
- 原生教师独立解题与某个主测试的输入及采样完全相同时复用该单元，并记录引用，不重复计为独立证据。
  ...
  此数以原生对照能精确复用为前提；如真实模板输入不一致，新增原生对照并在manifest记录实际数量，绝不强行去重。
...
- 公共GRPO教师本身在DAPO上训练过，本轮不能宣称该教师对这些题完全未见。
...
- [x] 固定资产并准备缺失模型。
- [x] 固定官方ModelScope文件元数据；GRPO复用历史哈希及缓存revision证据。
- [x] 实现跨协议输入、配对续写、原生对照去重、失败隔离和CPU测试。
- [x] 真实资产核验与GPU协议验证：七个模型28条原始短检查均完成，无生成think标签。
- [x] 启动不可变四卡队列并监督：00:57已自动转入正式诊断，见启动记录。
- [x] 全部单元验收、报告与双端归档：02:51完成27个单元，1920条正式输出及28条短检查；本地完整快照已核验135个产物哈希，并重算八对统计/置信区间/门槛，见[最终报告](../results/2026-09-23-qwen06-teacher-screen-final.md)。
