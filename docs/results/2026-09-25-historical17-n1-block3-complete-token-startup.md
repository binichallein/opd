...
恢复至Step2后正常退出0，四rank两份状态、优化器矩及64条原始轨迹均验收通过；
原Step1文件hash保持不变。`resume_gate.json`确认四rank scheduler从1推进到2。
这验证完整状态恢复，不声称vLLM生成或优化更新可逐位复现。

16:06:25队列自动启动正式Token，PID3888905；`resume_mode=disable`，恢复路径为空，
独立从原始学生初始化。16:22已完成正式Step2/100，正在继续训练；正式权重25/50/75/100尚未到达。
完整评测仍排在训练、保存审计和诊断绘图之后，不重复初始学生评测。

| 正式步骤 | 裁剪前梯度范数 | PG loss | 长度截断 | 周期尾部命中 | 平均输出token | 单步秒数 |
|---|---:|---:|---:|---:|---:|---:|
| 1 | 27.061 | 0.505 | 16/32 (50.0%) | 10/32 | 8525.40625 | 246.024 |
| 2 | 21.436 | 0.233 | 14/32 (43.75%) | 8/32 | 7672.65625 | 198.088 |

梯度、PG loss和耗时采用控制台精度，长度与周期尾部取实际原始轨迹，不用预检指标替代正式指标。
正式前两步的32x1数据顺序、prompt、采样参数与Block3逐步一致；首批response token也逐条相同。
正式Step1梯度精确值27.0613365，和预检27.0606003存在浮点差异，不能声称完整轨迹逐位可复现。
正式Step2与恢复预检Step2的输出和指标不同，记录严格分开。

正式Step1完整标量/位置快照检查无问题，非有限数与ratio上溢/下溢计数均为0，
sign flip、weighted sign flip及leakage均为0。正式完整诊断按Step1及每5步采集，
Step2只有每步训练日志及原始轨迹，不补造Step2完整位置诊断。
所读正式日志没有OOM、Traceback、CUDA error或ActorDiedError；仅两步不能证明长期稳定。

只读审计：[token_startup_1622.json](../../results/historical17_pair_n1_save25_20260925/token_startup_1622.json)。
预检、正式前64条原始轨迹、正式Step1位置/标量快照及Block3全部诊断图均已本地备份，
位于`supervision_1540/`下各自子目录，未覆盖预检文件。原始文件不进入Git。
训练保持运行，本次监督没有参数修复、热修改或重启；高截断/重复仍需与后续变化共同报告。
