...
\input{diagnostics_zh}

\section{历史证据及协议边界}
\label{app:history}
\begin{table}[htbp]
\centering\small
\caption{完整历史Step200 Avg@8对照（\%）。保留原始prompt和协议，仅在每个学生分层内比较方法；这些不是两组完整留存对照的额外训练seed。}
\label{tab:historyendpointsfull}
\input{generated/history_endpoints}
\end{table}
\input{history_notes_zh}
表~\ref{tab:historyendpointsfull}保留全部四组历史终点对照，主文表~\ref{tab:historyendpoints}选取其中两组Avg@8正向案例。
以下图表保留全部checkpoint变化。
\begin{figure}[p]
\centering
...
\end{figure}
\clearpage
\input{generated/history_all_zh}
...
\input{generated/history_probes}

\section{4B Base的全部Checkpoint结果}
\label{app:current}
\input{current_appendix_zh}
\begin{figure}[htbp]
\centering
...
\label{app:instruct}
\input{instruct_appendix_zh}

\section{逐步位置诊断}
\label{app:positionfigures}
\input{position_figures_zh}
