...
\input{generated/history_block10_diagnostics}
\end{table}
...
\paragraph{未报告或未运行的比较。}
没有完成文件的计划权重、advantage/ratio/reduction完整析因对照、额外训练seed和新增外部算法基线，
均不悄然计入已做实验。
缺失原始训练轨迹也不用后期重新采样冒充。
这些边界限制论文证据强度，却保留了计划、诊断probe、完成运行和独立复现之间的区别。
