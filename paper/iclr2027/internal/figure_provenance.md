...
生成使用内置OpenAI image_gen，进行了六次针对性连线/标签/边缘修正；最终检查公式、token数量、
...
第五次补回中间Block3到右侧目标函数框的红色箭头，闭合左侧外框右边界，并将黑色
反馈干线移至框外间隙。逐一核对两条目标输入连线和右侧单一更新箭头均完整保留。
第六次仅增强Rollout student和Frozen teacher两个小蓝框的右侧竖边。使用image_gen生成
修复候选，再仅将两条3像素宽的边缘条带合成回原图；逐像素检查共改变252像素，
指定条带外变化为0。其余边框、文字、数学公式和连线全部保持原像素。
