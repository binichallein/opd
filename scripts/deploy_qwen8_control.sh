...
# Transfer tracked control sources only; never replace the frozen training runtime.
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TARGET=/limx_embap/tos/user/Yaleon/opd_block_experiments_20260709/opd/analysis_deployments
COMMIT="$(git -C "${ROOT}" rev-parse HEAD)"
RELEASE="${TARGET}/${COMMIT}"
STAGE="${TARGET}/.qwen8_stage_${COMMIT}_$$"
...
ssh ml2 "set -euo pipefail
  ...
  test ! -e '${RELEASE}'
  mv -T '${STAGE}' '${RELEASE}'
  echo 'Immutable Qwen8 control release: ${RELEASE}; no GPU job launched'"
