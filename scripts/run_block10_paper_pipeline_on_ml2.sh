...
# This script is intended to run on the ml2 host.

ROOT="${ROOT:-/limx_embap/tos/user/Yaleon/opd_block_experiments_20260709/opd}"
RUN_ROOT="${RUN_ROOT:-${ROOT}/runs/20260709_block10_dapo17k_paper_qwen3}"
...
LOCAL_CACHE_ROOT="${LOCAL_CACHE_ROOT:-/limx_embap/tos/b10c}"
...
MATH_TEACHER="${MATH_TEACHER:-${ROOT}/models/Qwen3-4B-Base-GRPO}"
...
{
  "variant": "${variant}",
  "project_name": "opd_block10_dapo17k",
  ...
  "baseline_alignment": "Compare with completed token_opd and block3_mean from the paper-aligned Qwen3/DAPO run; train only block10_mean here"
}
JSON
...
main() {
  ...
  cleanup_before_training
  launch_train_variant block10_mean
  wait_for_finished_variant block10_mean "${TOTAL_TRAINING_STEPS}"
  cleanup_after_training_variant block10_mean
  run_eval_variant block10_mean "${TOTAL_TRAINING_STEPS}"
