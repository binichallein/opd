...
RUN_ROOT="${RUN_ROOT:-${ROOT}/runs/20260708_block3_dapo17k_paper_qwen3}"
...
LOCAL_CACHE_ROOT="${LOCAL_CACHE_ROOT:-/tmp/opd_block3_dapo17k_pipeline}"
...
{
  "variant": "${variant}",
  "project_name": "opd_block3_dapo17k",
  ...
  "baseline_alignment": "Blockwise/Rethinking-aligned Qwen3-1.7B-Base student, Qwen3-4B-Base-GRPO teacher, and raw 1,791,700-row DAPO-Math-17K pool"
}
JSON
...
main() {
  ...
  test -f "${TRAIN_DATA}"
  test -f "${VAL_DATA}"
  launch_train_variant token_opd
  wait_for_finished_variant token_opd "${TOTAL_TRAINING_STEPS}"
  cleanup_after_training_variant token_opd
  launch_train_variant block3_mean
  wait_for_finished_variant block3_mean "${TOTAL_TRAINING_STEPS}"
  cleanup_after_training_variant block3_mean
  run_eval_variant token_opd "${TOTAL_TRAINING_STEPS}"
  run_eval_variant block3_mean "${TOTAL_TRAINING_STEPS}"
