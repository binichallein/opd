...
def model_paths(step):
    if step not in STEPS:
        raise ValueError('Only the four completed Block3 checkpoints are authorized')
    return (TRAIN_ROOT / f'block3_mean/checkpoints/global_step_{step}/actor',
            RUN_ROOT / f'merged/block3_mean_step{step}')
...
def validate_training(state, acceptance, card):
    expected = {'source_commit': TRAIN_COMMIT, 'opd_prompt_protocol': PROTOCOL,
                'variant': 'block3_mean', 'student_model': str(assets.STUDENT),
    ...
    if (state.get('status') != 'complete' or state.get('checkpoint_steps') != sorted(STEPS)
        ...
        raise ValueError('Completed, accepted Qwen4 completion Block3 training is required')
...
def evaluate(step, runner, protected, control_commit):
    from transformers import AutoTokenizer
    shared.verify_hashes(protected)
    actor, model = model_paths(step)
    folder = RUN_ROOT / 'evaluations' / f'block3_mean_step{step}'
    ...
    runner([base.PYTHON, RUNTIME / 'external/revisiting_opd/scripts/model_merger.py', 'merge',
           ...
           RUN_ROOT / f'queue_jobs/merge_step{step}', job_env={'CUDA_VISIBLE_DEVICES': ''})
    ...
    jobs.write_json(folder / 'eval_card.json', {**shared.EVAL_VALUES, 'model': str(model),
        'checkpoint_step': step, 'training_source_commit': TRAIN_COMMIT, 'control_commit': control_commit,
...
def main():
    ...
    with (RUN_ROOT / 'queue.lock').open('a') as lock:
        ...
        try:
            ...
            training = TRAIN_ROOT / 'block3_mean'
            validate_training(base.read_json(TRAIN_ROOT / 'queue_state.json'),
                              base.read_json(training / 'acceptance.json'), base.read_json(training / 'run_card.json'))
            ...
            jobs.write_json(RUN_ROOT / 'queue_manifest.json', {
                ...
                'training_root': str(TRAIN_ROOT), 'eval_steps': STEPS, 'tasks': shared.TASK_COUNTS,
            ...
            results = {}
            for step in STEPS:
                results[step] = evaluate(step, runner, protected, commit)
